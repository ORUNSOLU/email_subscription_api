...
use email_subscription_api::telementry::{init_subscriber, get_subscriber}; 
...
// ensure the 'tracing stack' is initialized once using ONCE_CELL
...
async fn spawn_app() -> TestApp {
    // to initialize the TRACING LOG only once
    Lazy::force(&TRACING);
    ...
    configuration.database.database_name = Uuid::new_v4().to_string(); // new DB name for each connection
